...
- **Upstream status:** Not present in the currently fetched `upstream/main` at `ed5e17f4b`; upstream still filters listings through `.gitignore` in `apps/desktop/src/app/right-sidebar/files/ipc.ts`.
...
- **Upstream status:** Not present in the currently fetched `upstream/main` at `ed5e17f4b`; upstream has no `composer.dictation` keybind and its push-to-talk still leaves the transcript in the draft.
